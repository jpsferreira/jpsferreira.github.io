...
// Load publications and stats from JSON files (generated by build_publications.py:
// curated BibTeX synced from the jpsferreira/cv repo + OpenAlex citation counts)
async function loadPublications() {
	try {
		const [pubResponse, statsResponse] = await Promise.all([
			fetch('publications_bib/publications.json'),
			fetch('publications_bib/stats.json')
		]);
...
function renderPublication(pub) {
	...
	let badges = '';
	if (pub.id) {
		badges += `<span class="type-badge" title="${pub.category || ''}">${pub.id}</span>`;
	}
	if (year) {
